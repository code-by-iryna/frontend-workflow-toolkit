...
- Плагін нічого не деплоїть і не пушить сам — навіть Auto Mode обмежений
  ...
  **Виняток — `pr-review-resolver`:** він навмисно сам виконує `git commit`
  (по одному блоку зауважень за раз) і сам пише на GitHub (`gh api` —
  відповіді, реакції 👍), бо це і є суть роботи з чужим рев'ю. Кожна така
  дія — окреме, явне підтвердження в чаті для цього конкретного блоку/
  постингу, плюс технічний `gate-git.py` для самого `git commit`. `git
  push`, `gh pr create` і `gh pr merge` цей скіл так само не виконує — це
  лишається за тобою.
